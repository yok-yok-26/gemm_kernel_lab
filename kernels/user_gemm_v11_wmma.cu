...
__global__ void float_to_half_kernel_v11(const float* src, half* dst, int64_t count) {
...
__global__ void tile_seme_gemm_v11(
    const half* A,
    const half* B,
    float* C,
    int M, int N, int K
    ...
    for (int k0 = 0; k0 < K; k0 += 16) {
        const half* a_tile = A + (tile_m * 4 + idx_warp_m) * 16 * K + k0;
        const half* b_tile = B + k0 * N + (tile_n * 4 + idx_warp_n) * 16;
...
cudaError_t launch_user_gemm_v11_convert_inputs(const float* d_a, const float* d_b,
    ...
    float_to_half_kernel_v11<<<static_cast<int>((a_count + convert_block - 1) / convert_block),
                               convert_block, 0, stream>>>(d_a, d_a_half, a_count);
    CUDA_KERNEL_CHECK();
    float_to_half_kernel_v11<<<static_cast<int>((b_count + convert_block - 1) / convert_block),
...
cudaError_t launch_user_gemm_v11_wmma_only(const half* d_a_half, const half* d_b_half,
    ...
    tile_seme_gemm_v11<blockszie, tilesize_n, tilesize_m><<<grid, block, 0, stream>>>(
        d_a_half,
        d_b_half,
        d_c,
