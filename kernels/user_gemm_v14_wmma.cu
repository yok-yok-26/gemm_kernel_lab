...
__global__ void float_to_half_kernel_v14(const float* src, half* dst, int64_t count) {
...
__global__ void tile_seme_gemm_v14(
    const half* A,
    const half* B,
    float* C,
    int M, int N, int K
    ...
    cp_async_ca_zfill_16(
        &seme_a[read_flag][idx_t_m * 16 + idx_t_n * 8], 
        a_tile + idx_t_m * K + idx_t_n * 8, 
        16
    );

    cp_async_ca_zfill_16(
        &seme_b[read_flag][(idx_t_m / 16) * 16 * 16 + (idx_t_m % 16) * 16 + idx_t_n * 8], 
        b_tile + (idx_t_m % 16) * N + idx_t_n * 8 + (idx_t_m / 16) * 16, 
        16
    );
    ...
    for (int k0 = 16; k0 < (K + 16); k0 += 16) {
        ...
        if (k0 < K)
        {
            ...
            cp_async_ca_zfill_16(
                &seme_a[com_flag][idx_t_m * 16 + idx_t_n * 8], 
                a_tile_loop + idx_t_m * K + idx_t_n * 8, 
                16
            );

            cp_async_ca_zfill_16(
                &seme_b[com_flag][(idx_t_m / 16) * 16 * 16 + (idx_t_m % 16) * 16 + idx_t_n * 8], 
                b_tile_loop + (idx_t_m % 16) * N + idx_t_n * 8 + (idx_t_m / 16) * 16, 
                16
            );
            cp_async_commit_group();
        }
        
        const half* a_seme_tile = static_cast<const half*>(&seme_a[read_flag][warp_id * 16 * 16]);
        wmma::load_matrix_sync(a_frag, a_seme_tile, 16);
        for (int iloop = 0; iloop < 4; iloop++)
        {
            const half* b_seme_tile = static_cast<const half*>(&seme_b[read_flag][iloop * 16 * 16]);
...
cudaError_t launch_user_gemm_v14_convert_inputs(const float* d_a, const float* d_b,
    ...
    float_to_half_kernel_v14<<<static_cast<int>((a_count + convert_block - 1) / convert_block),
                               convert_block, 0, stream>>>(d_a, d_a_half, a_count);
    CUDA_KERNEL_CHECK();
    float_to_half_kernel_v14<<<static_cast<int>((b_count + convert_block - 1) / convert_block),
...
cudaError_t launch_user_gemm_v14_wmma_only(const half* d_a_half, const half* d_b_half,
    ...
    tile_seme_gemm_v14<blockszie, tilesize_n, tilesize_m><<<grid, block, 0, stream>>>(
        d_a_half,
        d_b_half,
        d_c,
