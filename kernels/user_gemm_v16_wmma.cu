...
__global__ void float_to_half_kernel_v16(const float* src, half* dst, int64_t count) {
...
__global__ void tile_seme_gemm_v16(
    const half* A,
    const half* B,
    float* C,
    int M, int N, int K
    ...
    int idx_t_m_b = tx / (16 / 4);    // [0, 32) 
    int idx_t_n_b = tx % (16 / 4);    // [0, 4)
    ...
    cp_async_ca_zfill_16(
        &seme_a[read_flag][
            idx_t_m * 32 + idx_t_n * 16 + 0
        ],
        a_tile + idx_t_m * K + idx_t_n * 16 + 0, 
        16
    );

    cp_async_ca_zfill_16(
        &seme_a[read_flag][
            idx_t_m * 32 + idx_t_n * 16 + 8
        ],
        a_tile + idx_t_m * K + idx_t_n * 16 + 8, 
        16
    );

    cp_async_ca_zfill_16(
        &seme_b[read_flag][
            idx_t_m_b * SEME_LEN_B + idx_t_n_b * 16 + 0
        ], 
        b_tile + idx_t_m_b * N + idx_t_n_b * 16 + 0, 
        16
    );

    cp_async_ca_zfill_16(
        &seme_b[read_flag][
            idx_t_m_b * SEME_LEN_B + idx_t_n_b * 16 + 8
        ], 
        b_tile + idx_t_m_b * N + idx_t_n_b * 16 + 8, 
        16
    );
    ...
    for (int k0 = 32; k0 < (K + 32); k0 += 32) {
        ...
        if (k0 < K)
        {
            ...
            cp_async_ca_zfill_16(
                ...
                a_tile_loop + idx_t_m * K + idx_t_n * 16 + 0, 
                16
            );
            ...
            cp_async_ca_zfill_16(
                ...
                a_tile_loop + idx_t_m * K + idx_t_n * 16 + 8, 
                16
            );
            ...
            cp_async_ca_zfill_16(
                ...
                b_tile_loop + idx_t_m_b * N + idx_t_n_b * 16 + 0, 
                16
            );
            ...
            cp_async_ca_zfill_16(
                ...
                b_tile_loop + idx_t_m_b * N + idx_t_n_b * 16 + 8, 
                16
            );
...
cudaError_t launch_user_gemm_v16_convert_inputs(const float* d_a, const float* d_b,
    ...
    float_to_half_kernel_v16<<<static_cast<int>((a_count + convert_block - 1) / convert_block),
                               convert_block, 0, stream>>>(d_a, d_a_half, a_count);
    CUDA_KERNEL_CHECK();
    float_to_half_kernel_v16<<<static_cast<int>((b_count + convert_block - 1) / convert_block),
...
cudaError_t launch_user_gemm_v16_wmma_only(const half* d_a_half, const half* d_b_half,
    ...
    tile_seme_gemm_v16<blockszie, tilesize_n, tilesize_m><<<grid, block, 0, stream>>>(
        d_a_half,
        d_b_half,
        d_c,
