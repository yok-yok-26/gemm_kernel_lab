...
__global__ void float_to_half_kernel_v17(const float* src, half* dst, int64_t count) {
...
__global__ void tile_seme_gemm_v17(
    const half* A,
    const half* B,
    float* C,
    int M, int N, int K
    ...
    int idx_t_m = tx < 64 ? tx64 / (1) : tx64 / (2);    // [0, 16) [16, 32) [32, 48) [48, 64) 
    int idx_t_n = tx < 64 ? tx64 % (1) : tx64 % (2);    // [0, 1)
    // int idx_t_m_b = tx / (2);    // [0, 32) 
    // int idx_t_n_b = tx % (2);    // [0, 2)
    ...
    if (tx < 64){
        cp_async_ca_zfill_16(
            &seme_a[read_flag][
                idx_t_m * 32 + idx_t_n * 32 + 0
            ],
            a_tile + idx_t_m * K + idx_t_n * 32 + 0, 
            16
        );

        cp_async_ca_zfill_16(
            &seme_a[read_flag][
                idx_t_m * 32 + idx_t_n * 32 + 8
            ],
            a_tile + idx_t_m * K + idx_t_n * 32 + 8, 
            16
        );

        cp_async_ca_zfill_16(
            &seme_a[read_flag][
                idx_t_m * 32 + idx_t_n * 32 + 16
            ],
            a_tile + idx_t_m * K + idx_t_n * 32 + 16, 
            16
        );

        cp_async_ca_zfill_16(
            &seme_a[read_flag][
                idx_t_m * 32 + idx_t_n * 32 + 24
            ],
            a_tile + idx_t_m * K + idx_t_n * 32 + 24, 
            16
        );
    }
    else{
        cp_async_ca_zfill_16(
            &seme_b[read_flag][
                idx_t_m * SEME_LEN_B + idx_t_n * 32 + 0
            ], 
            b_tile + idx_t_m * N + idx_t_n * 32 + 0, 
            16
        );

        cp_async_ca_zfill_16(
            &seme_b[read_flag][
                idx_t_m * SEME_LEN_B + idx_t_n * 32 + 8
            ], 
            b_tile + idx_t_m * N + idx_t_n * 32 + 8, 
            16
        );

        cp_async_ca_zfill_16(
            &seme_b[read_flag][
                idx_t_m * SEME_LEN_B + idx_t_n * 32 + 16
            ], 
            b_tile + idx_t_m * N + idx_t_n * 32 + 16, 
            16
        );

        cp_async_ca_zfill_16(
            &seme_b[read_flag][
                idx_t_m * SEME_LEN_B + idx_t_n * 32 + 24
            ], 
            b_tile + idx_t_m * N + idx_t_n * 32 + 24, 
            16
        );
    }
    ...
    for (int k0 = 32; k0 < (K + 32); k0 += 32) {
        ...
        if (k0 < K)
        {
            ...
            if (tx < 64){
                cp_async_ca_zfill_16(
                    ...
                    a_tile_loop + idx_t_m * K + idx_t_n * 32 + 0, 
                    16
                );
                ...
                cp_async_ca_zfill_16(
                    ...
                    a_tile_loop + idx_t_m * K + idx_t_n * 32 + 8, 
                    16
                );
                ...
                cp_async_ca_zfill_16(
                    ...
                    a_tile_loop + idx_t_m * K + idx_t_n * 32 + 16, 
                    16
                );
                ...
                cp_async_ca_zfill_16(
                    ...
                    a_tile_loop + idx_t_m * K + idx_t_n * 32 + 24, 
                    16
                );
            }
            else{
                cp_async_ca_zfill_16(
                    ...
                    b_tile_loop + idx_t_m * N + idx_t_n * 32 + 0, 
                    16
                );
                ...
                cp_async_ca_zfill_16(
                    ...
                    b_tile_loop + idx_t_m * N + idx_t_n * 32 + 8, 
                    16
                );
                ...
                cp_async_ca_zfill_16(
                    ...
                    b_tile_loop + idx_t_m * N + idx_t_n * 32 + 16, 
                    16
                );
                ...
                cp_async_ca_zfill_16(
                    ...
                    b_tile_loop + idx_t_m * N + idx_t_n * 32 + 24, 
                    16
                );
            }
        ...
        const half* a_seme_tile_2 = static_cast<const half*>(&seme_a[read_flag][warp_id * 32 * 16 + 16]);
        wmma::load_matrix_sync(a_frag, a_seme_tile, 32);
        wmma::load_matrix_sync(a_frag_2, a_seme_tile_2, 32);
        for (int iloop = 0; iloop < 4; iloop++)
        {
            const half* b_seme_tile = static_cast<const half*>(&seme_b[read_flag][iloop * 16]);
            const half* b_seme_tile_2 = static_cast<const half*>(&seme_b[read_flag][iloop * 16 + SEME_LEN_B * 16]);
            ...
            wmma::mma_sync(c_frag[iloop], a_frag_2, b_frag, c_frag[iloop]);
        }
...
cudaError_t launch_user_gemm_v17_convert_inputs(const float* d_a, const float* d_b,
    ...
    float_to_half_kernel_v17<<<static_cast<int>((a_count + convert_block - 1) / convert_block),
                               convert_block, 0, stream>>>(d_a, d_a_half, a_count);
    CUDA_KERNEL_CHECK();
    float_to_half_kernel_v17<<<static_cast<int>((b_count + convert_block - 1) / convert_block),
...
cudaError_t launch_user_gemm_v17_wmma_only(const half* d_a_half, const half* d_b_half,
    ...
    tile_seme_gemm_v17<blockszie, tilesize_n, tilesize_m><<<grid, block, 0, stream>>>(
        d_a_half,
        d_b_half,
        d_c,
