...
__global__ void float_to_half_kernel_v18(const float* src, half* dst, int64_t count) {
...
__global__ void tile_seme_gemm_v18(
    const half* A,
    const half* B,
    float* C,
    int M, int N, int K
...
cudaError_t launch_user_gemm_v18_convert_inputs(const float* d_a, const float* d_b,
    ...
    float_to_half_kernel_v18<<<static_cast<int>((a_count + convert_block - 1) / convert_block),
                               convert_block, 0, stream>>>(d_a, d_a_half, a_count);
    CUDA_KERNEL_CHECK();
    float_to_half_kernel_v18<<<static_cast<int>((b_count + convert_block - 1) / convert_block),
...
cudaError_t launch_user_gemm_v18_wmma_only(const half* d_a_half, const half* d_b_half,
    ...
    tile_seme_gemm_v18<blockszie, tilesize_n, tilesize_m><<<grid, block, 0, stream>>>(
        d_a_half,
        d_b_half,
        d_c,
