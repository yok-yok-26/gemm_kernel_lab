...
__global__ void tile_seme_gemm(float* a, float* b, float* c, int M, int K, int N){
  ...
  for (size_t istart = 0; istart < K; istart+=blockDim.x)
  {
    ...
    for (int itile = 0; itile < TILESIZE; itile++)
    {
      ...
      {
        b_seme[ty][tx * TILESIZE + itile] = b[N * (istart + ty) + idx_n + itile];
      }
      else
      {
        b_seme[ty][tx * TILESIZE + itile] = 0.0f;
      }
    }
    __syncthreads();    
    ...
    for (int iy = 0; iy < TILESIZE; iy++)
    {
      for (int ix = 0; ix < TILESIZE; ix++)
      {
        for (int ib = 0; ib < BLOCKSIZE; ib++){
          acc[iy][ix] += a_seme[ty * TILESIZE + iy][ib] * b_seme[ib][tx * TILESIZE + ix];
        }
      }
    }
    __syncthreads();
...
cudaError_t launch_user_gemm_v2(const float* d_a, const float* d_b, float* d_c,
  ...
  tile_seme_gemm<blockszie, tilesize><<<grid, block, 0, stream>>>(
