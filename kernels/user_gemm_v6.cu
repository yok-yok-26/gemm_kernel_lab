...
__global__ void tile_seme_gemm_v6(float* a, float* b, float* c, int M, int K, int N){
...
cudaError_t launch_user_gemm_v6(const float* d_a, const float* d_b, float* d_c,
  ...
  tile_seme_gemm_v6<blockszie, tilesize><<<grid, block, total_cnt * sizeof(float), stream>>>(
